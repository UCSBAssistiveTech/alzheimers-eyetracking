...
# Assuming FPS of 30 for velocity calculation, can be adjusted based on actual video frame rate
FPS = 30

# Blink detection parameters
alpha = 0.05  # smoothing factor for baseline
w1, w2, w3, w4 = 0.4, 0.3, 0.2, 0.1  # weights for blink score
threshold = 0.3  # blink threshold
N = 3  # consecutive frames for blink detection
...
def process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, debug_mode_on, render_cv_window, frame_number):
    ...
    cv2.putText(test_frame, "Frame: " + str(frame_number), (10,10), cv2.FONT_HERSHEY_SIMPLEX, .55, (255,255,255), 2)
    ...
    if final_contours and not isinstance(final_contours[0], list) and len(final_contours[0] > 5):
        ...
        cv2.putText(test_frame, "coords: " + str(center_x) + ', ' + str(center_y), (10,30), cv2.FONT_HERSHEY_SIMPLEX, .55, (255,90,30), 2) #center coordinates
...
def process_frame(frame):
    ...
    final_rotated_rect = process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, False, False, 0)
...
def process_video(video_path, input_method):
    ...
    csv_file = open('pupil_data.csv', 'w', newline='')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['frame_number', 'center_x', 'center_y', 'direction_x', 'direction_y', 'velocity_x', 'velocity_y', 'blink'])  # Write header row
    ...
    while True:
        ...
        #take the three images thresholded at different levels and process them
        pupil_rotated_rect = process_frames(thresholded_image_strict, thresholded_image_medium, thresholded_image_relaxed, frame, gray_frame, darkest_point, debug_mode_on, True, frame_number)
        
        # Get ellipse parameters
        ellipse = pupil_rotated_rect[0]
        if isinstance(ellipse, tuple) and len(ellipse) == 3:
            cx, cy = ellipse[0]
            minor_axis, major_axis = ellipse[1]
            angle = ellipse[2]
            pupil_detected = major_axis > 0
        else:
            pupil_detected = False
            cx, cy, major_axis, minor_axis, angle = 0, 0, 0, 0, 0
        ...
        if pupil_detected:
            ...
            if baseline_area is None:
                baseline_area = area
                baseline_aspect = aspect_ratio
            else:
                baseline_area = (1 - alpha)*baseline_area + alpha*area
                baseline_aspect = (1 - alpha)*baseline_aspect + alpha*aspect_ratio
        
        # Normalized area
        if pupil_detected and baseline_area:
            normalized_area = area / baseline_area
        else:
            normalized_area = 0.0
        normalized_area = np.clip(normalized_area, 0, 1)
        
        # Normalized aspect ratio
        if pupil_detected:
            aspect_ratio_val = minor_axis / major_axis
            norm_aspect = aspect_ratio_val / baseline_aspect if baseline_aspect else 1.0
        else:
            norm_aspect = 0.0
        norm_aspect = np.clip(norm_aspect, 0, 1)
        
        detection_failure = 0.0 if pupil_detected else 1.0
        
        # Vertical motion
        if pupil_detected and prev_cy is not None:
            dy = cy - prev_cy
        else:
            dy = 0.0
        prev_cy = cy if pupil_detected else prev_cy
        vertical_motion = abs(dy) / major_axis if pupil_detected else 0.0
        vertical_motion = np.clip(vertical_motion, 0, 1)
        
        # Blink score
        blink_score = w1 * (1 - normalized_area) + w2 * (1 - norm_aspect) + w3 * detection_failure + w4 * vertical_motion
        ...
        blink = 1 if blink_counter >= N else 0
        ...
        if prev_center is None:
            ...
            direction_x = center_x - prev_center[0]
            direction_y = center_y - prev_center[1]
        
        # Calculate velocity (assuming frame rate of 30 FPS)
        velocity_x = direction_x * FPS
        velocity_y = direction_y * FPS

        # Write to CSV
        csv_writer.writerow([frame_number, center_x, center_y, direction_x, direction_y, velocity_x, velocity_y, blink])
...
def select_video():
    ...
    video_path = 'C:/ATC/Eye Tracking/eye_test.mp4'
